...
function showResult(arr, chosenSet, uniqueArr) {
    ...
    arr.forEach((el) => {
        ...
        if (el.ok) {
            targetArea.innerHTML += `<div><span style="margin-top: 1em; padding-left: 1em; font-size: 1.5em;" class="card-text">${el.index}. ${el.name}</span> <p style="font-size: 0.8em; margin-left: 2em;">Ingredient: ${el.ingredients}</p></div>`
        } else {
            targetArea.innerHTML += `<div><i style="color: red;" class="fas fa-skull-crossbones"></i><span style="margin-top: 1em; font-size: 1.5em;" class="card-text"> ${el.index}. ${el.name}</span> <p style="font-size: 0.8em; margin-left: 2em;">Ingredient: ${el.ingredients}<p></div>`
        }
    })
}
